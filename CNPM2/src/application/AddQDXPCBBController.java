...
public class AddQDXPCBBController {
	@FXML
	TextField textField_soBB;
	@FXML
	...
	public AddQDXPCBBController(MenuQDController parent, Stage root) {
		this.root = root;
		this.parent = parent;
	...
	private void initialize() {
		textField_soBB.textProperty().addListener((observable, oldValue, newValue) -> {
			if (!Notifycation.allNumberReg.matcher(newValue).find())
				textField_soBB.pseudoClassStateChanged(Notifycation.errorClass, true);
			else
				textField_soBB.pseudoClassStateChanged(Notifycation.errorClass, false);
		});
		...
		BienBanAutoCompletion auto_cpl = new BienBanAutoCompletion(textField_soBB);
		auto_cpl.bind();
		...
		button_saveQD.setOnAction((e) -> {
			boolean check = true;
			if (textField_soBB.getText().isEmpty()) {
				check = false;
			}
			if (!Notifycation.allNumberReg.matcher(textField_soBB.getText()).find())
				check = false;
			...
			try {
				MenuController.serverServices.addQDXPHCK(Integer.parseInt(textField_soBB.getText()),
						UserProfile.profile.getMaCanBo(), java.sql.Date.valueOf(datePicker_ngayLap.getValue()),
						new AddQDXPCBB());
	...
	class AddQDXPCBB extends UnicastRemoteObject implements CallBackHandler {

		protected AddQDXPCBB() throws RemoteException {
			super();
			// TODO Auto-generated constructor stub
		}
